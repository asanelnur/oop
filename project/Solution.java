...
class Game{
    ...
    public void addPlayer(Player p){
        ...
        p=new MyPlayer(new Position(x, y),map);
        this.player=p;
    }
}
...
class MyPlayer implements Player{
    ...
    public MyPlayer(){};
    public MyPlayer(Position p,Map m){
        position=p;
        map=m;
    }
    ...
    @Override
    public void moveRight() throws InvalidMapException{
        int x=position.getX();
        int y=position.getY();
        if(y>=map.getSize()-1){
            throw new InvalidMapException("Not enough map elements");
        }else if(!map.list[x][y+1].equals("1")){
            position.setY(y+1);
        }
    }
    public void moveLeft() throws InvalidMapException{
        int x=position.getX();
        int y=position.getY();
        if(y<=0){
            throw new InvalidMapException("Not enough map elements");
    ...
    public void moveDown() throws InvalidMapException{
        int x=position.getX();
        int y=position.getY();
        if(x>=map.getRow()-1){
            throw new InvalidMapException("Not enough map elements");
        }else if(!map.list[x+1][y].equals("1")){
            position.setX(x+1);
        }
    }
    public void moveUp() throws InvalidMapException{
        int x=position.getX();
        int y=position.getY();
        if(x<=0){
            throw new InvalidMapException("Not enough map elements");
...
interface Player{
    public abstract void moveRight() throws InvalidMapException;
    public abstract void moveLeft() throws InvalidMapException;
    public abstract void moveDown() throws InvalidMapException;
    public abstract void moveUp() throws InvalidMapException;
